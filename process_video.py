...
for tif_stack_path in tif_files:
    ...
    ii, jj = vut.get_box_grid(box, angle, center=box.mean(axis=0))
    centroid = np.mean(box, axis=0)
    ii -= np.mean(ii)- centroid[1]
    jj -= np.mean(jj) - centroid[0]

    # Evaluate grid in all frames
    all_frame_vals = []
    values_grid_time = []
    for frame in tif_stack:
        values = vut.evaluate_image_in_grid(ii, jj, frame)
    ...
    traces = vut.get_displacements(all_frame_vals)
    ...
    chio.write_dfile(f'{fldr}/{name}_post_disp', save)
